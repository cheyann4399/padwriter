...
## 🧩 同源方法论的另一份实现：novel-manager skill

PadWriter 接入了一套**同源的「节拍器创作方法论」**的两份落地。核心思想一致：把长篇写作拆成可控「节拍」，用结构化索引（人设/世界观/节拍映射）约束 LLM，避免穿帮、剧透、变笨。

| | PadWriter（安卓 App） | novel-manager（Claude Code skill） |
|---|---|---|
| **运行端** | 安卓手机（全局悬浮球） | 电脑（Claude Code 会话） |
| **输入方式** | 语音按住→ASR→AI 润色→注入 | 文本交互：生成梗概 / 正文 / 处理收件箱 |
| **节拍控制** | 节拍器上下文控制（防穿帮/剧透/变笨） | 节拍索引 + 命中注入绑定设定（硬塞防线） |
| **写作管理** | WriterPad 模式：人设/世界观/大纲 | 三张索引表：人物/世界观/节拍 |
| **定位** | 随身手机端，边走边写 | 桌面端，工程化长文管理 |

本仓库的 `.claude/skills/novel-manager/` 是这套方法论的 **Claude Code 版本**——手机之外，你可以在电脑上用同一套思维写小说：往收件箱丢想法、生成卷级梗概、按节拍生成正文，中间的人设/世界观/节拍索引全由 agent 后台自动维护。上传后它可跨项目复用。
